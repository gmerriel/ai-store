...
3. **Desire-led**: Always reconnect with their original vehicle interest
...
- "hey {{first_name}}, random thought — have you seen what {{vehicle}} prices are doing right now? might be worth knowing about - {{sender_first_name}}"
- "{{first_name}}, quick one — something came across my desk about {{vehicle}} finance that made me think of you - {{sender_first_name}}"

### SOCIAL_PROOF (desire-led)
- "hey {{first_name}}, helped a couple of {{vehicle}} buyers this week — interesting whats available right now. worth a chat if youre still keen? - {{sender_first_name}}"
- "{{first_name}}, few people have been getting into {{vehicle}} recently — the deals are better than most people think. happy to run through it if youre interested - {{sender_first_name}}"

### VALUE_FIRST (desire-led, no CTA)
- "hey {{first_name}}, just a heads up — {{vehicle}} market has shifted a bit since we last chatted. might work in your favour actually - {{sender_first_name}}"
- "{{first_name}}, fyi the finance side for {{vehicle}} has changed a bit recently. good news if youre still thinking about it - {{sender_first_name}}"

### AUTHORITY (desire-led)
- "{{first_name}}, been doing a lot of {{vehicle}} deals lately — noticed a few things that could save you money. happy to share if youre interested - {{sender_first_name}}"

### PERMISSION (desire-led with soft close)
- "hey {{first_name}}, still keen on that {{vehicle}}? no stress if timings not right, just wanted to check in - {{sender_first_name}}"
- "{{first_name}}, just circling back on the {{vehicle}} — totally get it if lifes been busy. still here if you want to pick it back up - {{sender_first_name}}"

### URGENCY (factual, desire-led)
- "hey {{first_name}}, heads up — {{vehicle}} stock levels have been shifting lately. if youre still keen worth having a look sooner rather than later - {{sender_first_name}}"

## Email Examples (Ghosted Re-engagement)

Subject: "quick update on {{vehicle}} finance"
Body:
- Lead with their desire: "Still thinking about the {{vehicle}}?"
- Share something valuable: "The finance landscape has shifted since we last chatted..."
...
```json
{
  ...
  "tone_check": "[description of tone - should feel like a genuine check-in that reconnects with their vehicle desire]",
...
- [ ] LEADS with the vehicle desire (the hook is about THEIR vehicle, not about us stopping)
